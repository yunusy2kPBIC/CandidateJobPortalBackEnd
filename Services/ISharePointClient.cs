...
public interface ISharePointClient
{
    ...
    Task<string> ResolveColumnNameAsync(string listName, string expectedName, CancellationToken cancellationToken = default);
    ...
    Task<SharePointItemResponse> UploadResumeAsync(int candidateItemId, string candidateEmail, string filename, byte[] content, string contentType, string? uploadKey = null, CancellationToken cancellationToken = default);
